...
function Avatar({ blendShapes, isSpeaking }: { blendShapes: BlendShapeFrame[]; isSpeaking: boolean }) {
  const { scene, animations } = useGLTF('/models/programmers/programmerM.glb');
...
export default function AvatarPage() {
  ...
  const processVoiceInput = async (text: string) => {
    try {
      setIsLoading(true);
      setError(null);
      const response = await fetch('/api/programmer-chat/programmer-avatar', {
        method: 'POST',
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center justify-center p-4">
      <h1 className="text-3xl font-bold text-gray-800 mb-6">ProgrammerAI Avatar</h1>
